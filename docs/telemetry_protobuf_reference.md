...
For `mqtt-topic-router`, JetStream publishing uses this schema on `channel.data.mqtt`.
The router resolves active deployment identity through `/devices/internal` scoped by `device_owner`, and channel identity through `/device-deployments/{deployment_id}/channels/`.
